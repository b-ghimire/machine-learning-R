...
# This is an automated script and runs the classifier for different input parameter values
# Date: 19 May, 2010 
...
# next section contains calibration and prediction step

# reading calibration file
cat('Calibration text file name:', '\n')
calibration_file_name <- scan(what="character")
calibration_file_name_with_extension <- paste(calibration_file_name[1],".txt", sep="")
calibration_file <- read.table(calibration_file_name_with_extension, header=TRUE)

# reading validation file
cat('Validation text file name:', '\n')
...
# extract a subset from the data which is used as calibration
sub_train <- c(1:no_rows_calibration)

# extract a subset from the data which is used as validation
...
output_file_name <- paste(calibration_file_name, "_", "KappaValues.txt", sep="")
